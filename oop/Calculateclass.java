...
public class Calculateclass {
	...
	public void clacualte_ElapsedDays() {
		...
		if (is_LunaXLuna()) {
			go_LunaXLuna();
		} else if (is_LunaXSolar()) {
			go_LunaXSolar();
		} else if (is_SolarXLuna()) {
			go_SolarXLuna();
		} else if (is_SolarXSolar()) {
			go_SolarXSolar();
		}
		date[4] = elapsedDays;
	}

	private void go_SolarXSolar() {
		date[5] = 0;
		date[6] = 0;
		...
		int start_ElapsedDay = solaric.calculate_ElapsedDays(date[0], date[1], year);
		int finish_ElpsedDay = solaric.calculate_ElapsedDays(date[2], date[3], year);

		elapsedDays = finish_ElpsedDay - start_ElapsedDay;
	}

	private void go_LunaXSolar() {
		date[5] = 0;
		date[6] = 0;
		int start_date = array_Holidays[holidays.get_holidayIndex_From_Name(start_HolidayName)].get_hDay_date();
		int finish_date = array_Holidays[holidays.get_holidayIndex_From_Name(finish_HolidayName)].get_hDay_date();
		...
		date[3] = finish_date % 100;
		int start_ElapsedDay = solaric.calculate_ElapsedDays(date[0], date[1], year);
		int finish_ElapsedDay = solaric.calculate_ElapsedDays(date[2], date[3], year);

		elapsedDays = finish_ElapsedDay - start_ElapsedDay;
	}

	private void go_LunaXLuna() {
		date[5] = 0;
		date[6] = 0;
		int start_date = array_Holidays[holidays.get_holidayIndex_From_Name(start_HolidayName)].get_hDay_date();
		int finish_date = array_Holidays[holidays.get_holidayIndex_From_Name(finish_HolidayName)].get_hDay_date();
